...
void eliminarEstudiante() {
    ifstream archivo("estudiantesy.txt");
    ofstream temp("temp.txt");
    Estudiante e; 
    string buscado;
    bool eliminado=true;

    cout<<"Ingresa el numero de carnet del estudiante que deseas eliminar"<<endl;
    cin>>buscado;

    if (archivo.is_open() && temp.is_open())
    {
    while (archivo>>e.carnet>>e.nombre>>e.edad)
    {
    if (e.carnet==buscado)
    {
    ...
    temp.close();
        ...
        remove("estudiantes.txt");
        rename("estudiantes.txt","temp.txt");

        if (eliminado) {
            cout << "Eliminaste correctamente al estudiante que deseabas\n";
        } else {
            cout << "Estudiante no encontrado, vuelve a intentarlo\n";
        }
    } 
